...
class LennaGroundingDINOHead(DINOHead):
    ...
    def _get_targets_single(self, cls_score: Tensor, bbox_pred: Tensor,
        ...
        bbox_pred = bbox_pred * factor # xyxy, unnormalized
    ...
    def loss(self, hidden_states: Tensor, references: List[Tensor],
        ...
        if batch_mask != -1:
            new_loss_inputs = (loss_inputs[0][:, batch_mask, :, :].unsqueeze(1), loss_inputs[1][:, batch_mask, :, :].unsqueeze(1), \
                loss_inputs[2][batch_mask].unsqueeze(0), loss_inputs[3][batch_mask].unsqueeze(0), \
                [loss_inputs[4][batch_mask]], [loss_inputs[5][batch_mask]], \
                loss_inputs[6]
                )
            self.text_masks = text_token_mask[batch_mask].unsqueeze(0)
            losses = self.loss_by_feat(*new_loss_inputs)
        else:
